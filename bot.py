    ...
    async with aiosqlite.connect(DB_NAME) as db:
            ...
            if row:
                for period_idx, period_name in enumerate(["morning", "day", "evening"]):
                    if row[period_idx] != "OFF":
                        time_obj = datetime.strptime(row[period_idx], "%H:%M").time().replace(tzinfo=MSK_TZ)
                        job_queue.run_daily(
                            send_reminder,
                            time_obj,
                            chat_id=chat_id,
        ...
        async with db.execute(
            ...
            async for med_id, med_name, med_dosage, reminder_time in cursor:
                time_obj = datetime.strptime(reminder_time, "%H:%M").time().replace(tzinfo=MSK_TZ)
                job_queue.run_daily(
                    send_med_reminder,
                    time_obj,
                    chat_id=chat_id,
    ...
    try:
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            async with db.execute(
                ...
                high_bp_bad_feel = high_bp_bad_feel_row[0] if high_bp_bad_feel_row else 0
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "🟢 В норме • 🟡 Повышенное • 🟠 Высокое • 🔴 Крит. высокая • 🔵 Пониженное\n\n"
        
        "😊 <b>САМОЧУВСТВИЕ:</b>\n"
        "После каждого замера вы можете отметить как себя чувствуете:\n"
        "😊 Хорошо • 😐 Нормально • ☹️ Плохо\n"
...
async def med_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for med_id, med_name, med_dosage, reminder_time in meds:
        ...
        keyboard.append([InlineKeyboardButton(f"❌ Удалить {med_name}", callback_data=f"del_med_{med_id}")])
...
async def universal_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data.startswith("set_"):
            ...
            async with aiosqlite.connect(DB_NAME) as db:
                await db.execute(
                    f"UPDATE schedule SET {field}=? WHERE chat_id=?",
                    ("OFF", chat_id)
                )
                await db.commit()
        ...
        # --- Обработка самочувствия ---
            ...
            if rowid:
                async with aiosqlite.connect(DB_NAME) as db:
                    await db.execute(
                        "UPDATE records SET wellbeing=? WHERE rowid=?",
                        (feeling, rowid)
                    )
                    await db.commit()
...
async def log_measurement(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if wait_mode == "baseline":
        ...
        if match:
            ...
            await update.effective_message.reply_text(f"✅ Ваша норма {sys_val}/{dia_val} сохранена (вручную).")
        else:
    ...
    # --- Многоступенчатый диалог добавления лекарства ---
        ...
        try:
            ...
            await update.effective_message.reply_text("❌ Ошибка времени. Используйте формат ЧЧ:ММ.")
        return
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute("SELECT COUNT(*) FROM records WHERE chat_id=?", (chat_id,)) as count_cursor:
        ...
        await db.execute("DELETE FROM records WHERE chat_id=? AND timestamp < ?", (chat_id, cutoff_date))
        await db.commit()
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT id, name FROM medications WHERE chat_id=?",
            (chat_id,)
        ) as med_cursor:
...
if __name__ == "__main__":
    application = Application.builder().token(os.getenv("TG_TOKEN")).post_init(post_init).build()
