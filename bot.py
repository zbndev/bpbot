...
def schedule_user_jobs(chat_id: int, context: ContextTypes.DEFAULT_TYPE | Application):
    ...
    cursor.execute("SELECT morning, day, evening FROM schedule WHERE chat_id=?", (chat_id,))
    row = cursor.fetchone()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("INSERT OR IGNORE INTO schedule (chat_id, morning, day, evening) VALUES (?, ?, ?, ?)", 
...
async def set_time(update: Update, context: ContextTypes.DEFAULT_TYPE, period: str):
    if not context.args:
        await update.message.reply_text(f"Пожалуйста, укажите время. Пример: /set_{period} 08:30")
        return
    ...
    try:
        ...
        await update.message.reply_text("Неверный формат времени. Используйте ЧЧ:ММ (например, 08:30).")
        return
        
    chat_id = update.message.chat_id
    cursor.execute(f"UPDATE schedule SET {period}=? WHERE chat_id=?", (time_str, chat_id))
    conn.commit()
    schedule_user_jobs(chat_id, context)
    
    period_ru = {'morning': 'Утреннее', 'day': 'Дневное', 'evening': 'Вечернее'}[period]
    await update.message.reply_text(f"✅ {period_ru} напоминание успешно установлено на {time_str} (МСК).")
...
async def cmd_set_morning(update: Update, context: ContextTypes.DEFAULT_TYPE): await set_time(update, context, 'morning')
async def cmd_set_day(update: Update, context: ContextTypes.DEFAULT_TYPE): await set_time(update, context, 'day')
async def cmd_set_evening(update: Update, context: ContextTypes.DEFAULT_TYPE): await set_time(update, context, 'evening')

async def cmd_set_morning_off(update: Update, context: ContextTypes.DEFAULT_TYPE): await disable_time(update, context, 'morning')
async def cmd_set_day_off(update: Update, context: ContextTypes.DEFAULT_TYPE): await disable_time(update, context, 'day')
async def cmd_set_evening_off(update: Update, context: ContextTypes.DEFAULT_TYPE): await disable_time(update, context, 'evening')
...
async def log_measurement(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not re.match(r'^\d{2,3}\s*/\s*\d{2,3}(?:\s+\d{2,3})?$', text):
        await update.message.reply_text("⚠️ Неверный формат. Пожалуйста, используйте формат <code>120/80</code> или <code>120/80 85</code>.", parse_mode='HTML')
        return
    ...
    cursor.execute("DELETE FROM records WHERE chat_id=? AND timestamp < ?", (chat_id, seven_days_ago))
    conn.commit()
    
    await update.message.reply_text(f"✅ Записано: {text} (Время: {timestamp[-5:]} МСК)", parse_mode='HTML')
...
async def cmd_stats_3(update: Update, context: ContextTypes.DEFAULT_TYPE): await get_stats(update, context, 3)
async def cmd_stats_7(update: Update, context: ContextTypes.DEFAULT_TYPE): await get_stats(update, context, 7)
...
if __name__ == '__main__':
    ...
    if not TOKEN:
        logger.error("Критическая ошибка: Токен не найден. Убедитесь, что файл .env создан и содержит TG_TOKEN.")
